...
type Application = { id: string; company: string; role: string; status: ApplicationStatus; date: string; feedbackDate?: string; closedReason?: "no_answer"; url?: string; jobSnapshotId?: string; resumeVersionId?: string; coverVersionId?: string; note?: string; nextStep?: string; nextStepDate?: string };
...
export function JobSeekerApp({ onAccessRevoked }: JobSeekerAppProps = {}) {
  ...
  const replySummary = useMemo(() => summarizeReplies(applications.map((application) => ({
    ...application,
    trackName: normalizedTracks.find((track) => track.id === generatedDrafts.find((draft) => draft.id === application.resumeVersionId)?.trackId)?.name || "",
    ...
    fetch("/api/workspace", { cache: "no-store" })
      ...
      .then(({ response, data }) => {
        ...
        setApplications((current) => {
          ...
          if (closed.length) {
            setNotice(`${closed.length} ${closed.length === 1 ? "application was" : "applications were"} closed after ${SILENCE_DAYS} days with no answer: ${closed.slice(0, 3).map((row) => `${row.role} at ${row.company}`).join(", ")}${closed.length > 3 ? `, and ${closed.length - 3} more` : ""}. Reopen any of them by changing the status back.`);
          }
          return settled;
        });
      ...
      <section className="main-stage">
        ...
        {view === "applications" && applications.length > 0 && <section className={`reply-panel ${replySummary.ready ? "ready" : "waiting"}`}>
          <div>
            ...
            <p>{replySummary.reason}{replySummary.medianReplyDays != null ? ` Replies have taken ${replySummary.medianReplyDays} ${replySummary.medianReplyDays === 1 ? "day" : "days"} on average.` : ""}</p>
          </div>
          {replySummary.ready

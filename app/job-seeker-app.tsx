...
import { applicationForDraft, companyFromJobUrl, draftLinksFor, draftMatchesApplication, resolveApplicationDrafts } from "@/lib/application-links.mjs";
import { SILENCE_DAYS, expireSilentApplications, summarizeReplies } from "@/lib/application-lifecycle.mjs";
...
type Application = { id: string; company: string; role: string; status: ApplicationStatus; date: string; feedbackDate?: string; closedReason?: "no_answer"; silenceResetAt?: string; url?: string; jobSnapshotId?: string; resumeVersionId?: string; coverVersionId?: string; note?: string; nextStep?: string; nextStepDate?: string };
...
export function JobSeekerApp({ onAccessRevoked }: JobSeekerAppProps = {}) {
  ...
  // made; the track name is resolved here so the summary can group by it.
  const replySummary = useMemo(() => summarizeReplies(applications.map((application) => ({
    ...application,
    trackName: normalizedTracks.find((track) => track.id === resolveApplicationDrafts(application, generatedDrafts).resume.draft?.trackId)?.name || "",
  }))), [applications, generatedDrafts, normalizedTracks]);
  ...
  useEffect(() => {
    ...
    fetch("/api/workspace", { cache: "no-store" })
      ...
      .then(({ response, data }) => {
        ...
        // Silence has a deadline, checked once per load. Applications the owner
        // sent and nobody answered are closed with the reason on the row, so
        // the pipeline stops presenting dead roles as live ones — and so the
        // reply summary below is measured against what was really sent.
        let expiredByLoad: { id: string; company: string; role: string; waited: number }[] = [];
        setApplications((current) => {
          const { applications: settled, closed } = expireSilentApplications(current, { today: dateInputToday() });
          expiredByLoad = closed;
          return settled;
        });
        if (expiredByLoad.length) {
          setNotice(`${expiredByLoad.length} ${expiredByLoad.length === 1 ? "application was" : "applications were"} closed after ${SILENCE_DAYS} days with no answer: ${expiredByLoad.slice(0, 3).map((row) => `${row.role} at ${row.company}`).join(", ")}${expiredByLoad.length > 3 ? `, and ${expiredByLoad.length - 3} more` : ""}. Reopen any of them by changing the status back.`);
        }
  ...
  function saveDraftVersion(kind: "resume" | "cover") {
    ...
    if (target) {
      setApplications((current) => current.map((item) => item.id === target.id
        ? { ...item, resumeVersionId: kind === "resume" ? id : item.resumeVersionId, coverVersionId: kind === "cover" ? id : item.coverVersionId }
        : item));
      setGeneratedDrafts((current) => current.map((item) => item.id === id ? { ...item, applicationId: target.id } : item));
    }

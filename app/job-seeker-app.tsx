...
import { applicationForDraft, companyFromJobUrl, draftLinksFor, linkDraftToApplication, linkableDrafts, resolveApplicationDrafts, unpinDrafts } from "@/lib/application-links.mjs";
import { SILENCE_DAYS, applyStatusChange, describeSilentClosures, expireSilentApplications, summarizeReplies } from "@/lib/application-lifecycle.mjs";
...
// "Feedback" sits between Applied and Interview: a human answered — an email,
// a recruiter, a screen invitation — which is the first evidence that anything
// worked, and the only signal the pipeline had no way to record.
type ApplicationStatus = "Saved opportunity" | "Preparing" | "Applied" | "Feedback" | "Interview" | "Closed";
type Application = { id: string; company: string; role: string; status: ApplicationStatus; date: string; feedbackDate?: string; closedReason?: "no_answer" | "answered" | "not_sent"; silenceResetAt?: string; url?: string; jobSnapshotId?: string; resumeVersionId?: string; coverVersionId?: string; note?: string; nextStep?: string; nextStepDate?: string };
...
type GeneratedDraft = { id: string; type: "resume" | "cover"; title: string; content: string; createdAt: string; updatedAt: string; company: string; role: string; url?: string; trackId: string; jobSnapshotId?: string; applicationId?: string; sourceDocumentId?: string; origin: "generated" | "edited" | "uploaded"; provider?: AiProviderId | "local"; model?: string; modelLabel?: string; versionNumber?: number; usage?: CloudUsage; generationAttempts?: number; playbookRuleCount?: number; selectedPlaybookRuleCount?: number; approvedFactCount?: number; cleanedFactCount?: number; inputSummary?: string; reusedFromDraftId?: string; qualityScore?: number; qualityWordCount?: number; qualityBulletCount?: number; qualitySkillCount?: number };
...
export function JobSeekerApp({ onAccessRevoked }: JobSeekerAppProps = {}) {
  ...
  // A follow-up is due when its date has arrived and the application is still
  // live. Closed rows keep their date for the record but stop nagging.
  const followUpDue = useCallback((app: Application) =>
    Boolean(app.nextStepDate) && app.status !== "Closed" && (app.nextStepDate as string) <= dateInputToday(), []);
  const dueFollowUps = useMemo(() => applications.filter(followUpDue), [applications, followUpDue]);
  const filteredApplications = useMemo(() => applications.filter((app) => {
    ...
    return true;
    // Due follow-ups float to the top; everything else keeps its saved order
    // (Array.prototype.sort is stable, so ties never reshuffle).
  }).sort((left, right) => Number(followUpDue(right)) - Number(followUpDue(left))), [applications, followUpDue, pipelineFilters]);
  ...
  // Which résumé and cover letter each application shows, resolved once and
  // shared by the pipeline rows and the reply summary below.
  const resolvedDrafts = useMemo(() => new Map(applications.map((application) => [application.id, resolveApplicationDrafts(application, generatedDrafts)])), [applications, generatedDrafts]);

  // What has actually produced a reply. Every input is a decision the owner
  // made; the track comes from the résumé the row shows, pinned or matched.
  const replySummary = useMemo(() => summarizeReplies(applications.map((application) => ({
    ...application,
    trackName: normalizedTracks.find((track) => track.id === resolvedDrafts.get(application.id)?.resume.draft?.trackId)?.name || "",
  }))), [applications, normalizedTracks, resolvedDrafts]);
  ...
  // Silence has a deadline, checked once per load — after the restore above
  // has merged the durable backup, so it sees every application. It runs here
  // as plain code rather than inside a state updater: an updater is not run
  // when it is called once the component has other updates pending, so a
  // notice computed inside one is never shown.
  const silenceChecked = useRef(false);
  useEffect(() => {
    if (!workspaceLoaded || silenceChecked.current) return;
    silenceChecked.current = true;
    const { applications: settled, closed } = expireSilentApplications(applications, { today: dateInputToday() });
    if (!closed.length) return;
    setApplications(settled);
    setNotice(describeSilentClosures(closed, SILENCE_DAYS));
  }, [applications, setApplications, workspaceLoaded]);
  ...
  function saveApplication() {
    ...
    const snapshotCompany = company.trim() || companyFromJobUrl(roleUrl) || "Unknown company";
    ...
    const id = createId();
    const url = roleUrl.trim() || undefined;
    const fresh: Application = { id, company: snapshotCompany, role: snapshotRole, status: "Preparing", date: dateInputToday(), url, jobSnapshotId, note: "Saved from Role workspace" };
    // A résumé written before the application was saved belongs to it.
    const links = draftLinksFor(fresh, generatedDrafts);
    setApplications((current) => [{ ...fresh, ...links }, ...current]);
    const linkedIds = new Set(Object.values(links));
    if (linkedIds.size) setGeneratedDrafts((current) => current.map((draft) => linkedIds.has(draft.id) ? { ...draft, applicationId: id } : draft));
    const linked = Object.keys(links).length;
    setNotice(`Role saved to your application pipeline. It is not marked as submitted.${linked ? ` ${linked === 2 ? "Your résumé and cover letter are" : "Your résumé is"} linked to it.` : ""}`);
  }
  ...
  function saveDraftVersion(kind: "resume" | "cover") {
    ...
    // A posting URL names the employer on an ATS board, so an empty company
    // field is no longer stored as a placeholder that matches nothing.
    const draftCompany = company || companyFromJobUrl(roleUrl) || "Unknown company";
    const draftRole = role || "Untitled role";
    const draftUrl = roleUrl.trim() || undefined;
    ...
    // The application and the draft used to be written by two functions that
    // never spoke, so every row read "Résumé: Not linked" until the owner
    // linked it by hand — and the button that did that only appeared on an
    // exact company-and-role string match.
    const target = applicationForDraft({ id, type: kind, company: draftCompany, role: draftRole, url: draftUrl, jobSnapshotId }, applications);
    if (target) linkDraft(target.id, { id, type: kind });
    setNotice(`${kind === "resume" ? "Résumé" : "Cover letter"} version saved${target ? ` and linked to your ${target.company} application` : ""}.`);
  }

  // Both halves of the link, always written together: the application's
  // version id and the draft's application id. Two writers of this pair that
  // drifted apart were how every row came to read "Not linked".
  function linkDraft(applicationId: string, draft: Pick<GeneratedDraft, "id" | "type">) {
    setApplications((current) => linkDraftToApplication(current, [], applicationId, draft).applications);
    setGeneratedDrafts((current) => linkDraftToApplication([], current, applicationId, draft).drafts);
  }

  function attachDraftToApplication(applicationId: string, draft: GeneratedDraft) {
    linkDraft(applicationId, draft);
    setNotice(`${draft.type === "resume" ? "Résumé" : "Cover letter"} linked to this application.`);
  }

  function changeApplicationStatus(app: Application, nextStatus: ApplicationStatus) {
    const change = applyStatusChange(app, nextStatus, dateInputToday());
    setApplications((current) => current.map((item) => item.id === app.id ? applyStatusChange(item, nextStatus, dateInputToday()).application : item));
    if (change.answered) setNotice("Marked as answered. That is what V’s learns from — the date is recorded, so it can also tell you how long replies take.");
    else if (change.reopened) setNotice(`Reopened. If it stays on Applied with no answer, V’s waits another ${SILENCE_DAYS} days before closing it again.`);
    if (change.applied) setCompanies((current) => current.map((target) => target.status !== "Paused" && target.name.trim().toLowerCase() === app.company.trim().toLowerCase() ? { ...target, status: "Applied" } : target));
  }
  ...
  // The Target directory and the radar's own Targets tab were two separate
  // company lists that never spoke to each other: this one is a notebook the
  // owner maintains, that one is what actually gets scanned. Keeping both is
  // only defensible if a company can move from the notebook to the radar in one
  // press, which is what this does.
  //
  // The directory's four broad kinds map onto the radar's categories, which are
  // finer-grained; an agency lands on "Marketing Agency" because that is the
  // category the Bay Area agency pack uses.
  const RADAR_KIND_FOR: Record<CompanyTarget["kind"], string> = {
  ...
  async function watchOnRadar(target: CompanyTarget) {
    const careersUrl = target.careers.trim() || target.website.trim();
    if (!careersUrl) { setNotice(`Add a website or careers link to ${target.name} first — the radar needs somewhere to look.`); return; }
    setNotice(`Adding ${target.name} to the radar…`);
    try {
      const response = await fetch("/api/radar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "add_monitor", monitor: { company: target.name, kind: RADAR_KIND_FOR[target.kind], careersUrl, focus: target.focus, cadence: "daily" } }),
      });
      const data = await readJsonResponse<{ ok?: boolean; message?: string }>(response, "The company could not be added to the radar.");
      if (!response.ok || !data.ok) throw new Error(data.message || "The company could not be added to the radar.");
      setCompanies((current) => current.map((item) => item.id === target.id ? { ...item, status: "Monitoring" } : item));
      setNotice(`${target.name} is on the radar. Open Job radar → Targets to set how often it is checked.`);
    } catch (cause) {
      logError("radar", "add_monitor_failed", cause, { company: target.name });
      setNotice(cause instanceof Error ? cause.message : "The company could not be added to the radar.");
    }
  }
      ...
      <section className="main-stage">
        ...
        {view === "ai" && <section className="ai-reliability">
          <div className="ai-status-card">
            ...
            <div className="setup-note"><strong>The whole machine, end to end</strong>
              <p>
                <b>1 · Finding.</b> The radar scans your monitored companies every 2 hours from the app&rsquo;s own scheduler, and twice a day the automation bridge runs a full scan on GitHub&rsquo;s servers even if you never open the app. <b>2 · Telling you.</b> Every new matching role and every delivered draft is posted to the <a href="https://github.com/mpvaleta/valeta-job-seeker/issues" target="_blank" rel="noreferrer">📡 Radar digest issue ↗</a> in your private repository, and GitHub&rsquo;s ordinary notifications carry it to your inbox — no email login was given to anything. <b>3 · Drafting.</b> The scheduled Claude session drafts with your subscription — no API key — using exactly the model and tone chosen above, and delivers the finished résumé or cover letter straight into Data &amp; versions. <b>4 · Applying.</b> You review, the companion fills the form, and nothing is ever submitted without you.
              </p>
            </div>
        ...
        {view === "documents" && <section className="documents-workspace knowledge-workspace">
          <div className="document-import">
            ...
            <div className="link-source-import"><label className="source-url">Public article or YouTube URL <small>V’s reads a public article in full, and a YouTube video’s title and description. It never logs in, sends cookies, or reads private pages.</small><input type="url" value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://article… or https://youtu.be/…" /></label><div><button onClick={() => pasteFromClipboard(setSourceUrl, "source link")}>Paste link</button><button className="primary" onClick={readKnowledgeFromLink} disabled={sourceReading}>{sourceReading ? "Reading public source…" : "Read and import link"}</button></div></div>
        ...
        {view === "companies" && <section className="companies-workspace"><div className="target-form"><div className="step"><b>03</b><span>ADD A TARGET</span></div><h2>Companies, brands, and agencies — in one list.</h2><p>Your own notebook of companies worth following — it does nothing on its own. <b>Watch on radar</b> hands one to the Job radar, which then checks its careers page on a schedule. Use <b>Open job search</b> instead for LinkedIn, Indeed and the other boards no tool is allowed to read automatically.</p><label>Company or agency name<input value={companyName} onChange={(event) => setCompanyName(event.target.value)} placeholder="e.g. Agency, brand, or sports organization" /></label><div className="field-row"><label>Type<select value={companyKind} onChange={(event) => setCompanyKind(event.target.value as CompanyTarget["kind"])}><option>Brand</option><option>Agency</option><option>Sports</option><option>Tech</option></select></label><label>Focus<input value={companyFocus} onChange={(event) => setCompanyFocus(event.target.value)} /></label></div><label>Website<input value={companyWebsite} onChange={(event) => setCompanyWebsite(event.target.value)} placeholder="https://" /></label><label>Careers page<input value={companyCareers} onChange={(event) => setCompanyCareers(event.target.value)} placeholder="https://" /></label><button className="primary" onClick={addCompany}>Add to directory</button></div><div className="target-directory"><div className="review-heading"><div><span>TARGET DIRECTORY</span><h2>{companies.length} saved targets</h2></div><strong>Bay Area first</strong></div>{companies.length === 0 ? <div className="empty-state compact"><strong>Your target list starts here.</strong><span>Add brands, agencies, sports organizations, or tech teams. Nothing is marked as an open role until you verify it.</span></div> : <div className="company-list">{companies.map((target) => <article key={target.id}><div className="company-title"><span>{target.kind}</span><strong>{target.name}</strong><small>{target.market} · {target.focus || "No focus set"}</small></div><div className="company-links">{target.website && <a href={target.website} target="_blank" rel="noreferrer">Website</a>}{target.careers && <a href={target.careers} target="_blank" rel="noreferrer">Careers</a>}<button onClick={() => loadCompanyForRole(target)}>Use for role</button><button onClick={() => void watchOnRadar(target)} title="Adds this company to the Job radar, which checks its careers page on a schedule and files matching roles in the inbox.">Watch on radar</button></div><div className="company-actions"><label>Status<select value={target.status} onChange={(event) => setCompanies(companies.map((item) => item.id === target.id ? { ...item, status: event.target.value as CompanyTarget["status"] } : item))}><option>Researching</option><option>Monitoring</option><option>Applied</option><option>Paused</option></select></label><label>Notes<input value={target.notes} onChange={(event) => setCompanies(companies.map((item) => item.id === target.id ? { ...item, notes: event.target.value } : item))} placeholder="Contact, role, or next step" /></label><button onClick={() => setCompanies(companies.map((item) => item.id === target.id ? { ...item, status: "Paused" } : item))}>Pause</button></div></article>)}</div>}</div></section>}

        {view === "applications" && applications.length > 0 && <section className={`reply-panel ${replySummary.ready ? "ready" : "waiting"}`}>
          <div>
            <span>WHAT IS GETTING REPLIES</span>
            <h3>{replySummary.answered} answered · {replySummary.silent} silent{replySummary.replyRate != null ? ` · ${replySummary.replyRate}% reply rate` : ""}</h3>
            <p>{replySummary.reason}{replySummary.medianReplyDays != null ? ` The typical reply has taken ${replySummary.medianReplyDays} ${replySummary.medianReplyDays === 1 ? "day" : "days"} (median).` : ""}</p>
          </div>
          {replySummary.ready
            ? <div className="reply-signals">
              {replySummary.words.length > 0 && <p><b>Titles that answer:</b> {replySummary.words.map((entry) => `${entry.word} (${entry.count})`).join(", ")}. Worth having in your radar Target positions.</p>}
              {replySummary.tracks.length > 0 && <p><b>Résumé track behind the replies:</b> {replySummary.tracks.map((entry) => `${entry.name} (${entry.count})`).join(", ")}.</p>}
              {replySummary.words.length > 0 && <button onClick={() => copyText(replySummary.words.map((entry) => entry.word).join("\n"), setNotice)}>Copy these words for Search goals</button>}
            </div>
            : <div className="reply-signals"><p>Every count here comes from your own decisions — marking an application <b>Feedback</b> or <b>Interview</b> is what feeds it. Nothing is inferred, and nothing is shown until it would mean something.</p></div>}
        </section>}

        {view === "applications" && <section className="table-card"><div className="table-head"><div><span>PIPELINE</span><h2>{filteredApplications.length} of {applications.length} saved applications</h2>{dueFollowUps.length > 0 && <p className="followup-alert">{dueFollowUps.length} follow-up{dueFollowUps.length === 1 ? " is" : "s are"} due — the highlighted rows below are waiting on you.</p>}</div><button onClick={() => download("v-jobs-applications.json", JSON.stringify({ applications, generatedDrafts, jobSnapshots },null,2), "application/json")}>Export</button></div>{applications.length > 0 && <div className="radar-inbox-controls"><div className="radar-filters">{(["all", "Saved opportunity", "Preparing", "Applied", "Feedback", "Interview", "Closed"] as const).map((status) => <button key={status} className={pipelineFilters.status === status ? "selected" : ""} onClick={() => setPipelineFilters({ ...pipelineFilters, status })}>{status === "all" ? "All statuses" : status}</button>)}</div><label>Company<select value={pipelineFilters.company} onChange={(event) => setPipelineFilters({ ...pipelineFilters, company: event.target.value })}><option value="all">All companies</option>{pipelineCompanyOptions.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label>Search<input value={pipelineFilters.search} onChange={(event) => setPipelineFilters({ ...pipelineFilters, search: event.target.value })} placeholder="Role, company, or note" /></label></div>}{applications.length === 0 ? <div className="empty-state"><strong>No applications saved yet.</strong><span>Prepare a role in the workspace, then choose “Save to applications.”</span><button className="primary" onClick={() => setView("workspace")}>Prepare first role</button></div> : filteredApplications.length === 0 ? <div className="empty-state compact"><strong>No applications match these filters.</strong><span>Change the status, company, or search text.</span></div> : <div className="application-list">{filteredApplications.map((app) => { const resolved = resolvedDrafts.get(app.id) || resolveApplicationDrafts(app, generatedDrafts); const resumeDraft = resolved.resume.draft; const coverDraft = resolved.cover.draft; const matchingDrafts = linkableDrafts(app, generatedDrafts, resolved); return <article key={app.id} className={followUpDue(app) ? "followup-overdue" : ""}><div className="application-main"><strong>{app.role}</strong><span>{app.company}</span>{app.url && <a href={app.url} target="_blank" rel="noreferrer">Original role ↗</a>}<small>{app.note || "Saved opportunity — not submitted"}</small></div><label>Application date<input type="date" value={/^\d{4}-\d{2}-\d{2}$/.test(app.date) ? app.date : ""} onChange={(e) => setApplications((current) => current.map((item) => item.id === app.id ? {...item,date:e.target.value}:item))} /></label><label>Status<select value={app.status} onChange={(e) => changeApplicationStatus(app, e.target.value as ApplicationStatus)}><option>Saved opportunity</option><option>Preparing</option><option>Applied</option><option>Feedback</option><option>Interview</option><option>Closed</option></select></label><label>Note<input value={app.note || ""} onChange={(e) => setApplications((current) => current.map((item) => item.id === app.id ? {...item,note:e.target.value}:item))} placeholder="Follow-up, contact, or reminder" /></label><label>Next step<input value={app.nextStep || ""} onChange={(e) => setApplications((current) => current.map((item) => item.id === app.id ? {...item,nextStep:e.target.value}:item))} placeholder="e.g. Follow up with recruiter, prep for interview" /></label><label className={followUpDue(app) ? "followup-due" : ""}>Follow up on<input type="date" value={app.nextStepDate || ""} onChange={(e) => setApplications((current) => current.map((item) => item.id === app.id ? {...item,nextStepDate:e.target.value}:item))} title="When this date arrives, the row is highlighted and counted at the top until you act or clear it" /></label><div className="application-document-links"><span title={resolved.resume.linked ? "You linked this version to this application." : resumeDraft ? "Matched by job link, or by company and role. Use Link résumé below to pin a different version." : undefined}>Résumé: {resumeDraft ? `${resumeDraft.title}${resolved.resume.linked ? "" : " · matched"}` : "Not linked"}</span><span title={resolved.cover.linked ? "You linked this version to this application." : undefined}>Cover letter: {coverDraft ? `${coverDraft.title}${resolved.cover.linked ? "" : " · matched"}` : "Not linked"}</span></div><div className="application-actions"><button onClick={() => { setCompany(app.company); setRole(app.role); setRoleUrl(app.url || ""); setView("workspace"); setNotice("Role reopened. Its saved history remains intact."); }}>Open role</button><button onClick={() => changeApplicationStatus(app, "Closed")}>Archive as closed</button><button className={pendingApplicationDelete === app.id ? "danger-armed" : ""} onClick={() => { if (pendingApplicationDelete !== app.id) { setPendingApplicationDelete(app.id); setNotice(`Press Confirm delete to remove ${app.role} at ${app.company}. Anything else cancels it.`); return; } setApplications((current) => current.filter((item) => item.id !== app.id)); setGeneratedDrafts((current) => unpinDrafts(current, app.id)); setPendingApplicationDelete(""); setNotice("Application removed from your pipeline. Earlier private revisions still hold it — recover it in Data & versions."); }} onBlur={() => setPendingApplicationDelete((current) => current === app.id ? "" : current)}>{pendingApplicationDelete === app.id ? "Confirm delete" : "Delete"}</button></div>{matchingDrafts.length > 0 && <div className="application-draft-picker">{matchingDrafts.map((draft) => <button key={draft.id} onClick={() => attachDraftToApplication(app.id, draft)}>Link {draft.type === "resume" ? "résumé" : "cover letter"}: {draft.updatedAt}</button>)}</div>}</article>; })}</div>}</section>}

...
type SourceDocument = { id: string; title: string; type: string; importedAt: string; text: string; candidates: string[]; approved: string[]; status: "ready" | "needs-text" };
...
export function JobSeekerApp() {
  ...
  function importDocument() {
    ...
    setDocuments([{ id: crypto.randomUUID(), title, type: "Pasted text", importedAt: dateToday(), text: documentText, candidates: factCandidates(documentText), approved: [], status: "ready" }, ...documents]);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="main-stage">
        ...
        {view === "autofill" && <section className="autofill-grid"><div className="autofill-intro"><div className="step"><b>04</b><span>ASSISTED AUTOFILL</span></div><h2>Your data, ready for application forms.</h2><p>The browser companion maps common fields such as name, email, phone, location, LinkedIn, headline, and “about you.” It highlights anything sensitive or uncertain for your review.</p><div className="safety-list"><span>✓ Runs only when you click Fill</span><span>✓ Never presses Submit</span><span>✓ Never guesses legal or demographic answers</span><span>✓ Works from your approved profile</span></div></div><div className="autofill-package"><span>AUTOFILL PACKAGE</span><pre>{autofillData}</pre><div><button className="primary" onClick={() => copyText(autofillData,setNotice)}>Copy package</button><button onClick={() => download("valeta-autofill-profile.json",autofillData,"application/json")}>Download JSON</button></div><small>Next implementation: load this package into the Valeta Chrome companion included in the GitHub project, open an application page, and click Fill.</small></div></section>}
      </section>
    </main>
  );
}

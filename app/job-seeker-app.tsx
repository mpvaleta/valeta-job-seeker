...
type ApplicationRecommendation = { label: string; tone: "ready" | "edit" | "hold" | "start"; confidence: string; reason: string; actions: string[] };
...
export function JobSeekerApp() {
  ...
  const profileChecks = [profile.name, profile.email, profile.headline, profile.summary, facts.length >= 3 ? "facts" : ""];
  const profileReadiness = Math.round((profileChecks.filter(Boolean).length / profileChecks.length) * 100);
  const evidenceCoverage = requirements.length ? Math.round(((evidenceCounts.strong + evidenceCounts.partial * .5) / requirements.length) * 100) : 0;
  const sourceQuality = Math.min(100, (documents.length ? 35 : 0) + Math.min(45, facts.length * 9) + (documents.some((document) => document.approved.length) ? 20 : 0));
  const fit = jobText.trim().length < 80 ? 0 : Math.round(evidenceCoverage * .55 + profileReadiness * .25 + sourceQuality * .2);
  const recommendation = useMemo<ApplicationRecommendation>(() => {
    if (jobText.trim().length < 80) return { label: "Add the complete role", tone: "start", confidence: "Waiting for role details", reason: "A recommendation needs the complete job description, not only a title or URL.", actions: ["Paste the full job description", "Add the company and role title", "Run the evidence review"] };
    if (facts.length < 3) return { label: "Build evidence first", tone: "hold", confidence: "Low confidence", reason: "There are not enough approved career facts to make a reliable application recommendation yet.", actions: ["Upload your strongest résumé or GPT export", "Approve at least three verified facts", "Recheck this role"] };
    if (evidenceCoverage >= 65 && evidenceCounts.strong >= 1) return { label: "Prioritize and apply", tone: "ready", confidence: "High evidence confidence", reason: `${evidenceCounts.strong + evidenceCounts.partial} of ${requirements.length} detected requirements have approved support, including ${evidenceCounts.strong} strong ${evidenceCounts.strong === 1 ? "match" : "matches"}.`, actions: ["Open the tailored résumé and review wording", "Personalize the cover letter for the company", "Review every form field before submitting"] };
    if (evidenceCoverage >= 35) return { label: "Apply after targeted edits", tone: "edit", confidence: "Medium evidence confidence", reason: `The role has useful overlap, but ${evidenceCounts.gaps} ${evidenceCounts.gaps === 1 ? "requirement needs" : "requirements need"} stronger proof or careful omission.`, actions: ["Lead with the highest-scoring approved facts", "Find verified examples for partial matches", "Do not copy unsupported requirements into the résumé"] };
    return { label: "Hold and investigate", tone: "hold", confidence: "Low evidence confidence", reason: `The current fact bank does not support ${evidenceCounts.gaps} of ${requirements.length} detected requirements. This may be a stretch role or missing evidence.`, actions: ["Check whether your source documents contain missing proof", "Research the company and role scope", "Apply only if the gaps are preferences—not core requirements"] };
  }, [evidenceCounts, evidenceCoverage, facts.length, jobText, requirements.length]);
  const firstGap = evidenceMap.find((item) => item.strength === "Gap")?.requirement;
        ...
        {view === "workspace" && <div className="workspace-grid">
          ...
          <section className="output-card">
            ...
            {output === "analysis" ? <div className="analysis-view">
              <section className={`recommendation-card ${recommendation.tone}`}><div className="recommendation-top"><div><span>AI-ASSISTED RECOMMENDATION</span><h2>{recommendation.label}</h2></div><strong>{recommendation.confidence}</strong></div><p>{recommendation.reason}</p><ol>{recommendation.actions.map((action) => <li key={action}>{action}</li>)}</ol><div className="recommendation-actions"><button className="primary" onClick={() => setOutput("resume")}>Open tailored résumé</button><button onClick={saveApplication}>Save to pipeline</button></div><small>Based only on your approved facts, imported sources, and the pasted role. It never invents experience or predicts hiring.</small></section>

...
import { authenticatedEmail, clearCookie, exchangeLinkedInCode, getLinkedInConfig, LINKEDIN_SESSION_COOKIE, LINKEDIN_STATE_COOKIE, readCookie, secureCookie, verifyPayload } from "@/lib/linkedin-oauth";
import { createLinkedInSession } from "@/lib/oauth-store";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const session = await createLinkedInSession(getRuntimeDatabase(), state.owner, user, config.sessionSecret);
    destination.searchParams.set("linkedin", "connected");
    const response = NextResponse.redirect(destination);
    response.headers.append("Set-Cookie", secureCookie(LINKEDIN_SESSION_COOKIE, session.token, 7 * 24 * 60 * 60));
    response.headers.append("Set-Cookie", clearCookie(LINKEDIN_STATE_COOKIE));
    return response;
  } catch {

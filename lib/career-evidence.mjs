const CODE_MARKERS = [
  /[{}[\]<>]{2,}/,
  /\b(?:const|let|var|function|return|undefined|null|true|false|json|schema|stack|error|timestamp|requestid)\b/i,
  /(?:=>|===|!==|<\/?[a-z][^>]*>)/i,
];
...
export function prepareResumeEvidence(values, limit = 220) {
  ...
  return {
    facts: accepted.sort((left, right) => qualityScore(right) - qualityScore(left)).slice(0, limit),
    omitted,

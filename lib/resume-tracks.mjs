export const DEFAULT_RESUME_TRACKS = [
  ...
  },
  {
    ...
    headline: "Creative Strategy and Brand Operations Lead",
    summary: "",
    focus: ["creative strategy", "brand strategy", "creative lead", "creative manager", "content", "campaign", "storytelling", "design"],
  },
  {
    id: "sports",
    name: "Sports Marketing & Partnerships",
    headline: "Sports Marketing, Partnerships and Operations Lead",
    summary: "",
    focus: ["sports marketing", "sports", "partnerships", "sponsorship", "athlete", "fan", "experiential", "live event", "retention", "acquisition"],
  },
];

export function normalizeResumeTracks(value) {
  const saved = Array.isArray(value) ? value : [];
  const savedIds = new Set(saved.map((track) => clean(track?.id, 80)).filter(Boolean));
  // Migrations are additive: new built-in directions appear for existing users
  // without overwriting or removing a saved/custom track.
  const source = [...saved, ...DEFAULT_RESUME_TRACKS.filter((track) => !savedIds.has(track.id))];

...
export function buildBookmarklet({ mappingSource, captureSource = "", runtimeSource, data }) {
  ...
  const body = [
    ...
    mappingSource,
    // The extension's results-list reader, embedded for the same reason the
    // mapping rules are: on a phone there is no extension, and a second copy of
    // those selectors would drift the first time a board reskins.
    captureSource,
    runtimeSource,
    "})();",
  ].join("\n");

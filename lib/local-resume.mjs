...
// "Leading the creative team since 2021" names no end date at all, which
// DATE_RANGE requires. Read as an ongoing role starting from that point.
const SINCE_DATE = new RegExp(`\\bsince\\s+(${DATE_POINT})\\b`, "i");
...
// Deliberately broad, covering early-career and part-time roles alongside
// office/creative titles: a second concurrent job whose title this list
// misses does not just lose its own entry -- see the misattribution guard
// below -- so widening this list is the more complete fix.
const TITLE_WORD = /\b(?:manager|director|producer|coordinator|lead|specialist|analyst|engineer|designer|head|vp|vice\s+president|president|officer|associate|assistant|consultant|strategist|supervisor|executive|architect|editor|planner|buyer|recruiter|controller|principal|partner|chief|founder|owner|intern|technician|administrator|representative|advisor|writer|developer|scientist|marketer|generalist|barista|server|cashier|waiter|waitress|host|hostess|bartender|cook|chef|clerk|teller|tutor|nurse|driver|dispatcher|mechanic|electrician|plumber|paralegal|bookkeeper|receptionist|secretary|curator|librarian|translator|interpreter|auditor|therapist|counselor|dietitian|veterinarian|dentist|pharmacist|paramedic|firefighter|stylist|instructor|trainer|attendant|operator|agent|broker|underwriter|appraiser|inspector|photographer|videographer|artist|illustrator|copywriter|proofreader|merchandiser|stocker|dockworker|warehouse\s+associate|caregiver|nanny|babysitter)\b/i;
const COMPANY_HINT = /\b(?:inc|llc|ltd|corp|corporation|company|studios?|agency|agencies|group|media|partners?|labs?|works|collective|productions?|creative|communications?|consulting|solutions|systems|technologies|network|foundation|institute|associates|brands?|ventures|holdings|international|global|worldwide|cafe|café|restaurant|hospital|clinic|salon|market|hotel)\b\.?/i;
// Freelance and self-employed work legitimately has no proper-noun employer.
// "Freelance Producer, Various Clients" is standard résumé phrasing for it.
const INFORMAL_EMPLOYER = /^(?:various(?:\s+clients?)?|multiple\s+clients?|self[- ]employed|independent(?:ly)?|freelance)$/i;
...
export function buildLocalResume({ facts, roleText = "", headline = "", summary = "" }) {
  ...
  list.forEach((fact, index) => {
    ...
    // Attach by explicit employer mention, else to the open entry — but only
    // when that entry came from a standalone header, the layout where
    ...
    // A fact naming a company that is not the open entry's, or carrying its own
    // date range, is almost certainly a second job whose header this parser
    // failed to resolve (e.g. an unrecognized title word) — never absorb it as
    // a bullet under a different, already-open employer. That used to
    // misattribute an entire second job's work to the first one silently, with
    // no omission recorded. Losing the entry to "unplaced" is the safe failure.
    const namesADifferentCompany = Boolean(parsed.company) && (!openEntry || openEntry.company.toLowerCase() !== parsed.company.toLowerCase());
    if (!parsed.dates && !namesADifferentCompany && openEntry && openEntry.standalone) { openEntry.items.push(item); return; }
...
function parseDates(value) {
  ...
  const since = value.match(SINCE_DATE);
  if (since) {
    const startYear = Number(since[1].match(/\d{4}/)?.[0]);
    const startMonth = monthIndex(since[1].match(new RegExp(MONTH_NAME, "i"))?.[0]);
    if (!Number.isFinite(startYear)) return null;
    return { start: startYear, startMonth, end: 9999, endMonth: 12, text: `${startYear} – Present` };
  }
  return null;
}
...
function parseEmployment(value) {
  ...
  const withoutDates = value.replace(DATE_RANGE, " ").replace(SINCE_DATE, " ").replace(/\s+/g, " ").trim();
...
function parseHeaderLine(value, location) {
  ...
  const companySegment = rest.find((segment) => COMPANY_HINT.test(segment))
    || rest.find((segment) => INFORMAL_EMPLOYER.test(segment))

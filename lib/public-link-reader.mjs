...
// Bot-verification interstitials (Cloudflare, PerimeterX, DataDome,
// reCAPTCHA/hCaptcha) return HTTP 200 with a full HTML document -- distinct
// from the 401/403 "login required" case handled elsewhere in this file, and
// often long enough to pass the not-enough-text check too. Left undetected,
// the challenge page's own boilerplate silently becomes the "job
// description" or "article" the user imported, with no error at all --
// worse than a clear failure, since it looks like it worked.
//
// Titles are vendor boilerplate strings real job/article pages essentially
// never use, so an exact-ish match carries very low false-positive risk.
const BOT_CHALLENGE_TITLE = /^(?:just a moment|attention required|access denied|are you a robot\??|please verify you are a human|checking your browser|one more step|security check|verifying you are human|robot or human)\b/i;
// Implementation markers (script/element identifiers), not prose -- these
// are vendor-specific enough that a real page's own content wouldn't
// coincidentally contain them.
const BOT_CHALLENGE_MARKERS = /cf-chl-|cf_chl_|challenge-platform|id=["']challenge-(?:form|running)["']|g-recaptcha|grecaptcha|recaptcha\/api\.js|h-captcha|hcaptcha\.com|px-captcha|captcha-delivery\.com|_incapsula_resource|geo\.captcha-delivery\.com/i;

function looksLikeBotChallenge(title, html) {
  const cleanTitle = String(title || "").trim();
  if (cleanTitle && BOT_CHALLENGE_TITLE.test(cleanTitle)) return true;
  return BOT_CHALLENGE_MARKERS.test(String(html || "").slice(0, 20_000));
}
...
// The full description as YouTube stores it in the player payload. The meta
// description tag carries a truncated copy, which is why this is read first.
export function extractYouTubeDescription(html) {
  const match = /"shortDescription":"((?:[^"\\]|\\.)*)"/.exec(String(html || ""));
  if (!match) return "";
  try {
...
export async function readPublicLink(value, options = {}) {
  ...
  const page = isHtml ? extractPublicPage(fetched.body, fetched.finalUrl) : {
  ...
  if (isHtml && looksLikeBotChallenge(page.title, fetched.body)) {
    throw new PublicLinkError("bot_challenge_detected", "This page returned a bot-verification challenge instead of its real content. Open the page yourself in a browser, let it finish loading, then paste the visible text here.", 422);
  }
  // A company homepage can be intentionally sparse while still exposing a valid Careers,
  // Jobs, or Opportunities link. Preserve those links for Radar discovery instead of
  // treating the page as unreadable.
  if (page.text.length < 80 && !page.jobs.length && !page.links.length) {
...
/*
 * A YouTube video as a knowledge source.
 *
 * The transcript is still attempted first, and still preferred. It usually no
 * longer arrives: YouTube's timedtext endpoint answers a server-side request
 * with HTTP 200 and an empty body — measured on five videos in September 2026,
 * with creator captions and automatic ones alike, with a browser User-Agent, a
 * Referer, and through the innertube player API, which answers UNPLAYABLE.
 * Serving captions now needs a proof-of-origin token bound to a real browser
 * session, and forging one is bot-protection circumvention rather than a fix.
 *
 * So the import no longer fails outright. The video's own description is real
 * public text — often the summary, the chapters and the links — and it is
 * imported under its own source type, so nothing is ever mistaken for a
 * transcript. Only a video with nothing readable at all is refused, and then
 * the message says what actually happened and how to paste the transcript.
 */
const YOUTUBE_MAX_FETCH_BYTES = 3_000_000;
const YOUTUBE_MIN_DESCRIPTION = 200;
...
async function readYouTube(requestedUrl, videoId, fetchImpl) {
  ...
  // A watch page is 1.2-1.7 MB of player JSON, over the ordinary article
  // ceiling. A video import must not fail for the size of YouTube's own page.
  const fetched = await fetchTextFollowingRedirects(watchUrl, fetchImpl, YOUTUBE_MAX_FETCH_BYTES);
  const page = extractPublicPage(fetched.body, fetched.finalUrl);
  const title = page.title.replace(/\s*-\s*YouTube\s*$/i, "") || `YouTube video ${videoId}`;
  const description = extractYouTubeDescription(fetched.body) || page.description || "";
  const spoken = await readYouTubeCaptions(fetched.body, fetchImpl);
  const common = {
    requestedUrl: requestedUrl.href,
  ...
  if (spoken) {
    return {
      ...
      metadata: {
        ...
        captions: spoken.kind === "asr" ? "automatic" : "creator-provided",
      },
    };
  }
  if (description.length >= YOUTUBE_MIN_DESCRIPTION) {
    return {
      ...common,
      sourceType: "youtube-description",
      text: description.slice(0, MAX_EXTRACTED_TEXT),
      metadata: { videoId, language: "unknown", captions: "unavailable" },
    };
  }
  throw new PublicLinkError(
    "youtube_transcript_unavailable",
    "YouTube no longer hands video transcripts to apps — it answers with an empty file however the video is captioned — and this video's description is too short to import on its own. Open the video, use “…more” then “Show transcript”, copy it, and paste it in as text.",
    422,
  );
}

// Returns the spoken text, or null. Never throws: a video whose captions
// cannot be read is not a failed import, it is an import without a transcript.
async function readYouTubeCaptions(html, fetchImpl) {
  try {
    ...
    return { transcript, language: String(track.languageCode || "unknown"), kind: track.kind };
  } catch {
    return null;
  }
}

async function fetchTextFollowingRedirects(initialUrl, fetchImpl, maxBytes = MAX_FETCH_BYTES) {
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
    ...
    if (declaredLength > maxBytes) throw new PublicLinkError("page_too_large", "This page is too large to import safely.", 413);
...
function cleanText(value, maxLength) {
  return decodeHtml(String(value || "")).replace(/\s+/g, " ").trim().slice(0, maxLength);
}

...
// A shortlist or an application is the strongest signal the radar ever gets:
// the user did not just tolerate the role, they acted on it. Words that recur
// across those roles — and are NOT already in the saved targets, which score
// through the profile itself — describe what the user actually goes for,
// which the saved profile often trails behind.
//
...
export function deriveInterestSignal(pursuits, profileValue) {
  ...
  if (rows.length < INTEREST_MIN_SAMPLE) {
    return {
      ...
      words: [],
      categories: [],
      reason: `V's learns from roles you shortlist or apply to once there are at least ${INTEREST_MIN_SAMPLE}. So far: ${rows.length}.`,
      stats,
    };
  }
  ...
  return {
    ...
    categories,
    reason: words.length || categories.length
      ? `Learned from ${rows.length} roles you shortlisted or applied to.`
...
export function scoreRadarOpportunity(opportunity, profileValue, dismissalSignal, interestSignal) {
  ...
  if (pursued.boost) {
    ...
    reasons.push(pursued.matched.length
      ? `similar to roles you shortlisted or applied to: ${pursued.matched.slice(0, 3).join(", ")}`

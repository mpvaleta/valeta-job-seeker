...
/** Whether a URL carries an applicant-tracking system's own posting id. */
export function hasAuthoritativeJobId(value) {
  const raw = clean(value, 4_000);
  if (!raw) return false;
  try {
    const url = new URL(raw);
    url.hostname = url.hostname.toLowerCase().replace(/^www\./, "");
    return Boolean(atsIdentity(url));
  } catch {
    return false;
  }
}
 ...
 *
 * A posting that already carries an ATS posting id gets no content key at all.
 * Employers really do open two requisitions with the same title in the same
 * place — Jack Morton runs several identical warehouse roles, Perplexity had
 * two openings for one engineering title — and each is a separate job someone
 * can apply to. The board's own id settles it, so guessing from the text there
 * would merge away real openings rather than duplicates.
 */
export function opportunityContentKey(job = {}) {
  if (hasAuthoritativeJobId(job.sourceUrl)) return "";

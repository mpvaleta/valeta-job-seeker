...
const TRUSTED_JOB_SOURCES = new Set(["apple", "ashby", "google-careers", "greenhouse", "icims", "imported", "lever", "linkedin-saved", "meta-job", "public-job-page", "recruitee", "smartrecruiters", "structured-job-page", "teamwork-online", "v-watch", "workable", "workday"]);
...
export function deriveDismissalSignal(dismissals, profileValue) {
  ...
  const words = rankByCount(wordCounts, DISMISSAL_MIN_REPEATS).slice(0, 12);
  ...
  // ordinary noise.
  const categories = rankByCount(categoryCounts, Math.max(DISMISSAL_MIN_REPEATS, Math.ceil(teaching.length / 2))).slice(0, 4);
...
// Learning from what the user pursues.
//
// The mirror image of dismissal learning, read with the same conservatism.
// A shortlist, an application, or a link the user imported by hand are the
// strongest signals the radar ever gets: the user did not just tolerate the
// role, they acted on it. Words that recur across those roles — and are NOT
// already in the saved targets, which score through the profile itself —
// describe what the user actually goes for, which the saved profile often
// trails behind.
//
//   - Nothing is learned until several pursued roles exist.
//   - A word must recur across separate pursued roles to count.
//   - The boost is bounded and additive only: it can lift a borderline role
//     into view, but it can never override an exclusion, the location gate,
//     or the startups-only filter, all of which fail `passes` outright.
const INTEREST_MIN_SAMPLE = 3;
const INTEREST_MIN_REPEATS = 2;
const INTEREST_MAX_BOOST = 15;

export function deriveInterestSignal(pursuits, profileValue) {
  const profile = normalizeRadarProfile(profileValue);
  const rows = (Array.isArray(pursuits) ? pursuits : []).filter((row) => row && clean(row.title, 300));

  // Words the saved targets already contain earn their points through title
  // and skill matching; counting them again here would double-reward them.
  ...
  for (const row of rows) {
    ...
    // Count each word once per role, so one long title cannot outvote the rest.
    for (const word of new Set(tokens(title))) {
      if (word.length < 4 || knownWords.has(word)) continue;
      wordCounts.set(word, (wordCounts.get(word) || 0) + 1);
    }
    const category = clean(row.companyCategory, 120);
    if (category) categoryCounts.set(category, (categoryCounts.get(category) || 0) + 1);
  ...
  const stats = { pursuitsRead: rows.length, knownWords: knownWords.size };
  if (rows.length < INTEREST_MIN_SAMPLE) {
    return {
      ready: false,
      words: [],
      categories: [],
      reason: `V's learns from roles you pursue — shortlist, apply to, or import yourself — once there are at least ${INTEREST_MIN_SAMPLE}. So far: ${rows.length}.`,
      stats,
    };
  }

  const words = rankByCount(wordCounts, INTEREST_MIN_REPEATS).slice(0, 12);
  // Same bar as dismissed categories: only a category the user pursues more
  // often than not says anything about taste.
  const categories = rankByCount(categoryCounts, Math.max(INTEREST_MIN_REPEATS, Math.ceil(rows.length / 2))).slice(0, 4);
  ...
  return {
    ...
    categories,
    reason: words.length || categories.length
      ? `Learned from ${rows.length} roles you shortlisted, applied to, or imported yourself.`
      : `Nothing recurred often enough across ${rows.length} pursued roles to learn from yet.`,
    stats,
  };
}

// Separated from the derivation for the same reason as dismissalPenalty:
// scoring stays a pure function and the boost is testable on its own.
export function interestBoost(titleLower, companyCategory, signal) {
  if (!signal || !signal.ready) return { boost: 0, matched: [] };
  ...
  if (!matched.length && !categoryHit) return { boost: 0, matched: [] };
  const boost = Math.min(INTEREST_MAX_BOOST, matched.length * 5 + (categoryHit ? 6 : 0));
  return { boost, matched, categoryHit };
}
...
// Shared by both learners: entries meeting the minimum count, most frequent
// first, ties broken alphabetically so results are stable across runs.
function rankByCount(counts, minimum) {
...
export function scoreRadarOpportunity(opportunity, profileValue, dismissalSignal, interestSignal) {
  const profile = normalizeRadarProfile(profileValue);
  ...
  const exactTitleMatches = profile.titles.filter((target) => titleMatches(titleLower, target));
  ...
  if (locationMismatch) {
    ...
    reasons.push(location
      ? `outside the target market (${location.slice(0, 60)}) — location filter`
      : "no location listed — location filter");
  }
  ...
  // What the user has shortlisted or applied to. Additive only: it can lift a
  // borderline role into view, but `passes` still fails on any hard gate, so
  // the boost never overrides an exclusion, the location filter, or the
  // startups-only filter.
  const pursued = interestBoost(titleLower, companyCategory, interestSignal);
  if (pursued.boost) {
    score += pursued.boost;
    reasons.push(pursued.matched.length
      ? `similar to roles you pursued: ${pursued.matched.slice(0, 3).join(", ")}`
      : "company type you usually pursue");
  }
  ...
  return {
    ...
    passes: bounded >= profile.minScore && !exclusions.length && !stageMismatch && !locationMismatch,
  };
}
...
export function detectCareerSource(value) {
  ...
  // Workable hosts every board at apply.workable.com/<account>; the same
  // account token also names the public widget API the reader uses.
  if (host === "apply.workable.com" && firstPath && firstPath !== "api") return { type: "workable", token: firstPath, url };
  // Recruitee boards live on a tenant subdomain with a public /api/offers/
  // feed. The bare product site (www/careers/docs) is not a tenant board.
  if (host.endsWith(".recruitee.com") && host.split(".").length === 3) {
    const tenant = host.split(".")[0];
    if (tenant && !["www", "careers", "docs", "app", "api", "help", "support", "blog"].includes(tenant)) return { type: "recruitee", token: tenant, url };
  }
...
async function tryCareerSource(scanUrl, purpose, target, fetchImpl) {
  try {
    ...
    else if (source.type === "workable") jobs = await readWorkable(source.token, fetchImpl);
    else if (source.type === "recruitee") jobs = await readRecruitee(source.token, fetchImpl);
...
async function readWorkday(token, url, fetchImpl) {
  ...
  const jobs = [];
  // The first page used to be the whole read, so any employer with more than
  // 100 open postings had the rest silently truncated. Three pages is a
  // bounded widening: large boards surface far more roles, and a board that
  // fits in one page still costs exactly one request because a short page
  // ends the loop.
...
// Workable's widget API is the same public feed its embedded careers widgets
// read; per-job descriptions are not included there, so description stays
// empty and scoring leans on the title, as with Workday.
async function readWorkable(token, fetchImpl) {
  const payload = await fetchJson(`https://apply.workable.com/api/v1/widget/accounts/${encodeURIComponent(token)}`, fetchImpl);
  return (Array.isArray(payload?.jobs) ? payload.jobs : []).map((job) => ({
    title: job.title,
    location: [job.city, job.state, job.country].filter(Boolean).join(", ") || (job.telecommuting ? "Remote" : ""),
    description: stripMarkup(job.description || ""),
    sourceUrl: job.url || (job.shortcode ? `https://apply.workable.com/${encodeURIComponent(token)}/j/${encodeURIComponent(job.shortcode)}/` : ""),
    sourceType: "workable",
    datePosted: job.published_on || "",
  })).filter((job) => job.sourceUrl);
}

async function readRecruitee(token, fetchImpl) {
  const payload = await fetchJson(`https://${encodeURIComponent(token)}.recruitee.com/api/offers/`, fetchImpl);
  return (Array.isArray(payload?.offers) ? payload.offers : []).map((offer) => ({
    ...
    description: stripMarkup(offer.description || ""),
    sourceUrl: offer.careers_url || (offer.slug ? `https://${token}.recruitee.com/o/${offer.slug}` : ""),
  ...
  if (followCareerHub) {
    for (const hub of rankCareerLinks(page.links).filter((link) => link.href !== page.finalUrl && link.href !== page.requestedUrl).slice(0, 5)) {
      try {
        ...
        else if (source.type === "workable") discovered = await readWorkable(source.token, fetchImpl);
        else if (source.type === "recruitee") discovered = await readRecruitee(source.token, fetchImpl);
          ...
          : /opportunit/.test(haystack) ? 40
            : /openings?|open roles?/.test(haystack) ? 35
              : /join(?:-|\s)?us|work(?:-|\s)?with(?:-|\s)?us/.test(haystack) ? 30
                : 0;
      const ats = /greenhouse|lever|ashby|workday|smartrecruiters|workable|recruitee|jobvite/.test(haystack) ? 20 : 0;
...
function atsIdentity(url) {
  ...
  if (host === "apply.workable.com") {
    // /<account>/j/<shortcode>, optionally followed by /apply.
    const [account, marker, shortcode] = segments;
    if (account && marker === "j" && shortcode) return `workable:${account.toLowerCase()}:${shortcode.toLowerCase()}`;
  }

  if (host.endsWith(".recruitee.com")) {
    // <tenant>.recruitee.com/o/<slug>, optionally followed by /c/new.
    const at = segments.indexOf("o");
    const slug = at >= 0 ? segments[at + 1] : "";
    const tenant = host.split(".")[0];
    if (tenant && slug) return `recruitee:${tenant}:${slug.toLowerCase()}`;
  }

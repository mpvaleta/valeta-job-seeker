...
const TRUSTED_JOB_SOURCES = new Set(["apple", "ashby", "google-careers", "greenhouse", "lever", "meta-job", "smartrecruiters", "structured-job-page", "v-watch", "workday"]);
const NAVIGATION_TITLE = /^(?:apply|browse|careers?|details?|discover|explore|find|job search|jobs?|join us|learn|life at|locations?|open roles?|openings?|opportunities|our teams?|read more|search|search jobs?|search roles?|students?|talent community|teams?|view|view all|work(?:ing)? (?:at|with).*)$/i;
const NAVIGATION_PHRASE = /\b(?:browse (?:all )?(?:jobs|roles)|career opportunities|explore (?:jobs|opportunities|roles)|find (?:a |your )?(?:job|role)|job search|learn about working|learn more|life at|search (?:all )?(?:jobs|openings|positions|roles)|talent community|view (?:all )?(?:jobs|openings|positions|roles)|working at)\b/i;
...
export function classifyRadarOpportunity(opportunity = {}, target = {}) {
  ...
  return {
    trackId,
    trackLabel,
    companyCategory: classifyCompanyCategory(target?.kind || target?.companyType, combined),
  };
}
...
export function isPlausibleRadarJob(job = {}) {
  ...
  if (clean(job?.description, 2_000).length >= 80 && JOB_DETAIL_PATH.test(sourceUrl)) return true;
  return JOB_DETAIL_PATH.test(sourceUrl);
}

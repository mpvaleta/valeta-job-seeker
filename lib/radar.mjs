...
export async function discoverTargetJobs(target, options = {}) {
  ...
  const careersUrl = clean(target?.careersUrl || target?.careers || "", 4_000);
  if (!careersUrl) throw new Error("Add a public careers page before scanning this target.");
  if (isLinkedInUrl(careersUrl)) throw new Error("LinkedIn does not permit automated monitoring. Use the company’s public careers page or official ATS board instead.");
  const source = detectCareerSource(careersUrl);
  let jobs;
  ...
  else jobs = await readGenericCareerPage(source.url.href, fetchImpl);
  ...
  return uniqueBy(jobs.map((job) => ({
    ...
    sourceUrl: safeJobUrl(job.sourceUrl, careersUrl),
...
async function readGenericCareerPage(url, fetchImpl) {

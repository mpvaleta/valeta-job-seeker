...
// The one market that is not optional. This is a Bay Area search; every other
// market is something the owner opts into on top of it, never instead of it.
// Keeping the spelling here rather than in the component means the scorer and
// the UI cannot drift apart about what the home market is called.
export const RADAR_HOME_MARKET = "San Francisco Bay Area";

// The markets offered as one-click toggles. Anything outside this list can
// still be typed in by hand — the list is a shortcut, not a restriction.
export const RADAR_MARKETS = [
  { id: "bay-area", label: "SF Bay Area", value: RADAR_HOME_MARKET, home: true },
  ...
  { id: "united-states", label: "Anywhere in the US", value: "United States" },
];

// Guarantees the home market survives whatever the caller assembled, however it
// happens to be spelled ("Oakland" and "Bay Area" both already mean it).
export function withHomeMarket(locations) {
  const values = cleanList(locations, 20, 120);
  return values.some((item) => isBayAreaLocation(item)) ? values : [RADAR_HOME_MARKET, ...values];
}
...
const TRUSTED_JOB_SOURCES = new Set(["apple", "ashby", "captured", "google-careers", "greenhouse", "icims", "imported", "lever", "linkedin-saved", "meta-job", "public-job-page", "smartrecruiters", "structured-job-page", "teamwork-online", "v-watch", "workday"]);
...
/*
 * What the user marked "no longer available".
 *
 * A closed listing is not a rejection — it is the opposite. The user opened
 * that role, wanted it enough to check, and found the employer had taken it
 * down. So it teaches in the other direction from a dismissal: the words in
 * those titles describe work worth surfacing sooner next time.
 *
 * Words the saved goals already contain are skipped rather than counted. They
 * are scored by the goals themselves; counting them here would pay twice for
 * the same evidence and let a handful of closed roles quietly outrank the
 * profile. What is left is exactly the vocabulary the user has shown interest
 * in but never wrote down.
 *
 * The bonus is small on purpose. Learning nudges the ranking; the gates below
 * still decide what is even a candidate.
 */
const CLOSED_MIN_SAMPLE = 3;
const CLOSED_MIN_REPEATS = 2;
const CLOSED_MAX_BONUS = 8;

export function deriveClosedListingSignal(closedRows, profileValue) {
  const profile = normalizeRadarProfile(profileValue);
  const rows = (Array.isArray(closedRows) ? closedRows : []).filter((row) => row && row.reason === "listing_closed");
  const alreadyWanted = new Set(profile.titles.flatMap((title) => disciplineWords(title)));
  const counts = new Map();
  const companies = new Map();
  for (const row of rows) {
    for (const word of new Set(disciplineWords(clean(row.title, 300)))) {
      if (alreadyWanted.has(word)) continue;
      counts.set(word, (counts.get(word) || 0) + 1);
    }
    const company = clean(row.company, 160);
    if (company) companies.set(company, (companies.get(company) || 0) + 1);
  }
  const stats = { closedRead: rows.length, wordsTracked: counts.size };
  if (rows.length < CLOSED_MIN_SAMPLE) {
    return {
      ready: false,
      words: [],
      companies: [],
      reason: `V's learns from roles you mark "no longer available" once there are at least ${CLOSED_MIN_SAMPLE}. So far: ${rows.length}.`,
      stats,
    };
  }
  const byCount = (map) => [...map.entries()]
    .filter(([, count]) => count >= CLOSED_MIN_REPEATS)
    .sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))
    .map(([value]) => value);
  const words = byCount(counts).slice(0, 8);
  return {
    ready: words.length > 0,
    words,
    // Not scored — surfaced so the user can see which employers keep posting
    // work they wanted and act on it themselves.
    companies: byCount(companies).slice(0, 6),
    reason: words.length
      ? `Learned from ${rows.length} roles you marked "no longer available".`
      : `Nothing recurred often enough across ${rows.length} closed roles to learn from yet.`,
    stats,
  };
}

export function closedListingBonus(titleLower, signal) {
  if (!signal || !signal.ready) return { bonus: 0, matched: [] };
  const found = disciplineWords(titleLower);
  const matched = (signal.words || []).filter((word) => found.some((item) => sameDiscipline(item, word)));
  if (!matched.length) return { bonus: 0, matched: [] };
  return { bonus: Math.min(CLOSED_MAX_BONUS, matched.length * 4), matched };
}
...
const DISCIPLINE_GROUPS = [
  ...
  { id: "activation", test: /^activations?$/ },
  { id: "sponsorship", test: /^sponsorships?$/ },
  { id: "traffic", test: /^traffic(?:king)?$/ },
  { id: "workflow", test: /^workflows?$/ },
  { id: "editorial", test: /^editorial$/ },
  { id: "broadcast", test: /^broadcast(?:ing)?$/ },
  { id: "advertising", test: /^advertis(?:ing|ement|ements)$/ },
];
...
/*
 * The same work, written in someone else's vocabulary.
 *
 * sameDiscipline() above only ever merges spellings of one word. That leaves
 * the gate literal: an employer who calls the job a "Program Manager" or a
 * "Delivery Lead" is describing exactly the work a saved "Project Manager"
 * names, and the radar was throwing those postings out as off-target — the
 * single largest source of missed roles once the collection queue was fixed.
 *
 * Each row lists words that name one line of work in different houses. They are
 * deliberately narrow: relatedness is symmetric within a row and is NOT
 * transitive across rows, so "workflow" bridging the first two rows never makes
 * "project" and "operations" the same thing. Nothing here merges two careers
 * that merely sound alike — "product" appears in no row at all, which is what
 * keeps the product/producer separation intact.
 */
const RELATED_DISCIPLINES = [
  ["project", "program", "delivery", "traffic", "workflow"],
  ["operations", "studio", "workflow", "process"],
  ["production", "content", "broadcast"],
  ["event", "activation", "sponsorship"],
  ["marketing", "brand", "campaign", "advertising", "communications"],
];

const RELATED_DISCIPLINE_INDEX = new Map();
for (const row of RELATED_DISCIPLINES) {
  for (const word of row) {
    const related = RELATED_DISCIPLINE_INDEX.get(word) || new Set();
    for (const other of row) if (other !== word) related.add(other);
    RELATED_DISCIPLINE_INDEX.set(word, related);
  }
}

function relatedDiscipline(left, right) {
  if (left === right) return false;
  return Boolean(RELATED_DISCIPLINE_INDEX.get(left)?.has(right));
}
...
//   "adjacent" — the posting uses a different word for the same work
//                ("Program Manager" against a saved "Project Manager"). Worth
//                far fewer points than a family match, but it passes the role
//                gate: the words differ, the job does not.
...
export function titleRelevance(titleValue, targetTitles, targetSkills = []) {
  ...
  // Same work, different word. Reported as "their word ≈ your word" so the
  // inbox can say which saved target the posting was read against.
  const adjacent = [];
  for (const word of found) {
    const target = wanted.find((item) => relatedDiscipline(word, item));
    if (target) adjacent.push(`${word} ≈ ${target}`);
  }
...
// Inside a phrase, these words are the same word. Substituting them produces
// the ways an employer is likely to have written the skill the user saved:
// "creative operations" also reads as "creative ops", "project management" as
// "program management". Every list keeps the user's own word first, so the
// phrase they typed is always the first variant tried.
const PHRASE_SYNONYMS = new Map(Object.entries({
  operations: ["ops", "operational"],
  ops: ["operations"],
  operational: ["operations"],
  management: ["mgmt", "manager"],
  manager: ["management", "mgmt"],
  project: ["program", "programme"],
  projects: ["programs", "programmes"],
  program: ["programme", "project"],
  programs: ["programmes", "projects"],
  production: ["producing"],
  producer: ["production lead"],
  integrated: ["end-to-end", "omnichannel"],
...
// A phrase expands into at most this many variants, and the expansion is
// truncated after every word rather than at the end — otherwise a six-word
// skill with three options per word would build 729 strings to throw away.
const PHRASE_VARIANT_CAP = 12;
const phraseVariantCache = new Map();
const phrasePatternCache = new Map();

function boundedCache(cache, key, value) {
...
export function phraseVariants(phrase) {
  const normalized = clean(phrase, 200).toLowerCase().replace(/\s*&\s*/g, " and ").trim();
  if (!normalized) return [];
  const cached = phraseVariantCache.get(normalized);
  if (cached) return cached;
  let variants = [[]];
  for (const word of normalized.split(/\s+/)) {
    const options = [word, ...(PHRASE_SYNONYMS.get(word) || [])];
    const next = [];
    for (const prefix of variants) for (const option of options) next.push([...prefix, option]);
    variants = next.slice(0, PHRASE_VARIANT_CAP);
  }
  // A variant with no letters or digits left in it would compile to a pattern
  // that matches every haystack, so it never reaches the matcher.
  return boundedCache(phraseVariantCache, normalized, [...new Set(variants.map((parts) => parts.join(" ")))].filter((variant) => /[a-z0-9]/.test(variant)));
}

// Whole-phrase match, tolerant of the punctuation employers actually use:
// hyphens and en dashes stand in for spaces, and "&" reads as "and".
function phraseRegExp(variant) {
  const cached = phrasePatternCache.get(variant);
  if (cached) return cached;
  const parts = variant.split(/[\s\-–—]+/).filter(Boolean)
    .map((part) => part === "and" ? "(?:and|&)" : escapeRegExp(part));
  return boundedCache(phrasePatternCache, variant, new RegExp(`(?:^|[^a-z0-9])${parts.join("[\\s\\-–—]+")}(?:[^a-z0-9]|$)`, "i"));
}

function skillPresent(haystack, skill) {
  const variants = phraseVariants(skill);
  if (!variants.length) return false;
  return variants.some((variant) => phraseRegExp(variant).test(haystack));
}

export function scoreRadarOpportunity(opportunity, profileValue, dismissalSignal, closedSignal) {
  const profile = normalizeRadarProfile(profileValue);
  ...
  } else if (relevance.tier === "adjacent" && relevance.matched.length) {
    // Worth less than a family match and deliberately so: the employer's word
    // means what the user's word means, but nothing in the title is literally
    // what they asked for.
    score += Math.min(14, 8 + relevance.matched.length * 3);
    reasons.push(`same work, different word: ${relevance.matched.slice(0, 3).join(", ")}`);
  }
  ...
  // Roles like the ones the user marked "no longer available". Applied before
  // the gates below, never after: the gates cap the score, and a bonus added on
  // top of a cap would lift a gated role above the ceiling that gate exists to
  // impose.
  const missed = closedListingBonus(titleLower, closedSignal);
  if (missed.bonus) {
    score += missed.bonus;
    reasons.push(`like roles you missed: ${missed.matched.slice(0, 3).join(", ")}`);
  }
...
// Discovery keeps every plausible posting up to this many per read. It must
// stay above the largest monitored board (OpenAI lists ~770, Anthropic ~570):
// the store treats a board read as complete when it stamps
// last_listing_read_at, so a read cut short here would flag the unseen tail
// as "no longer listed" while those roles are still live.
export const DISCOVERY_JOB_CAP = 1_000;
...
export async function discoverTargetJobsDetailed(target, options = {}) {
  ...
  })).filter(isPlausibleRadarJob), (job) => `${job.sourceUrl}|${job.title}`).slice(0, DISCOVERY_JOB_CAP);

...
function tokens(text) {
  return rawWords(text)
    .map(stem)
    .filter((word) => word.length > 2 && !stopWords.has(word));
}

// Stems drive counting so "campaign"/"campaigns" merge, but each keyword is
// displayed as its most frequent original word — never a bare stem.
export function keywords(text, limit = 14) {
  const counts = new Map();
  const originals = new Map();
  for (const raw of rawWords(text)) {
    const stemmed = stem(raw);
    if (stemmed.length <= 2 || stopWords.has(stemmed) || stopWords.has(raw)) continue;
    counts.set(stemmed, (counts.get(stemmed) || 0) + 1);
    const variants = originals.get(stemmed) || new Map();
    variants.set(raw, (variants.get(raw) || 0) + 1);
    originals.set(stemmed, variants);
  }
  return [...counts.entries()]
    .sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))
    .slice(0, limit)
    .map(([stemmed]) => [...originals.get(stemmed).entries()].sort((left, right) => right[1] - left[1] || left[0].length - right[0].length)[0][0]);
}
...
export function extractRequirements(text, limit = 10) {
  ...
  const candidates = text
    .replace(/\r/g, "")
    .split(/\n+|(?<=[.!?])\s+/)
    .map((item) => item.replace(/^[-•*\s]+|^\d{1,2}[.)]\s+/, "").replace(/\s+/g, " ").trim())
    ...
    .filter((item) => !/\b(?:is hiring|is seeking|is looking for|we (?:are|’re|'re) (?:hiring|seeking|looking))\b/i.test(item))
    .filter((item) => {
...
export function analyzeRole({ jobText = "", facts = [], profile = {}, sources = [] }) {
  ...
  const evidenceMap = requirements.map((requirement) => {
    const evidence = cleanFacts
      ...
      // A single shared common word is noise, not support. "Own paid media
      // buying across programmatic platforms" matched a campaign-budgets fact
      // on one token and was reported as partially supported, which is exactly
      // the kind of claim the evidence rules exist to prevent.
      .filter((item) => item.shared >= 2 || item.score >= .25)
    ...
    // A gap carries no evidence, so nothing can be quoted in support of it.

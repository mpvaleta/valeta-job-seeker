 ...
 * owner moving the status back off Closed — resets the clock via
 * `silenceResetAt`, so the same 60 days can be waited out again rather than
 * closing the row again on the very next load.
 */
...
export function expireSilentApplications(applications, options = {}) {
  ...
  const next = rows.map((application) => {
    ...
    const waited = daysSince(application.silenceResetAt || application.date, today);
...
export function summarizeReplies(applications, options = {}) {
  ...
  return {
    ...
    words: ready ? words : [],
    tracks: ready ? tracks : [],
    medianReplyDays: ready ? medianReplyDays : null,
    reason: ready

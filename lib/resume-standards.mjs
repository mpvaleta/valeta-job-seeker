...
/*
 * The numbers in a line that assert something checkable: "35%", "$1.2M",
 * "12 designers". Normalized so "$1,200" and "1200" compare equal, since the
 * question is whether the quantity exists in the evidence, not how it is
 * punctuated. Years are excluded — date ranges are legitimately reformatted
 * by every generator, and the timeline is checked structurally elsewhere.
 */
...
export function auditResume(text, options = {}) {
  ...
  // ---- Truthfulness: claims must exist in the approved evidence ----
  //
  // The structured generators already trace every bullet to fact indexes, but
  // the text on screen can be hand-edited, pasted from the resume-tailor
  // skill, or reworded by a provider — and none of those paths re-verify. This
  // checks the two claim types that most often drift: numbers (a metric that
  // grew in the retelling) and employers (a company the evidence never names).
  // Findings ride the evidence dimension for display but never move its score:
  // they are review flags, and only the user can judge a paraphrase.
  const approvedFacts = (Array.isArray(options.approvedFacts) ? options.approvedFacts : [])
    .map((fact) => String(fact || "").trim())
    .filter(Boolean);
  if (approvedFacts.length) {
    const factNumbers = new Set(approvedFacts.flatMap((fact) => numericClaims(fact)));
    const factsText = approvedFacts.join("\n").toLowerCase();
    const flaggedNumbers = new Set();
    for (const line of [summary, ...bullets]) {
      for (const claim of numericClaims(line)) {
        if (factNumbers.has(claim) || flaggedNumbers.has(claim)) continue;
        flaggedNumbers.add(claim);
        findings.push({
          dimension: "evidence",
          severity: "high",
          message: `The number "${claim}" does not appear in any approved fact. Verify it against your real history or remove it — a metric that grew in editing is exactly what the fact bank exists to prevent.`,
          line,
        });
      }
    }
    for (const entry of entries) {
      const employer = String(entry.employer || "").trim();
      if (employer.length < 3) continue;
      if (!factsText.includes(employer.toLowerCase())) {
        findings.push({
          dimension: "evidence",
          severity: "high",
          message: `The employer "${employer}" does not appear in your approved evidence. Check the spelling against your fact bank, or approve the evidence that names it before applying with this draft.`,
        });
      }
    }
  }
